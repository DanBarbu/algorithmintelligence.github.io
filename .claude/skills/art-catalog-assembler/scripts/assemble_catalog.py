...
(from build_mapping.py, optionally hand-edited) and:
  ...
  3. Leaves any UNMATCHED_PLACEHOLDER rows untouched (still shows the
...
def main():
    ...
    with open(args.mapping_csv, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row["flag"] in ("UNMATCHED_PLACEHOLDER", "UNUSED_IMAGE") or not row["image_file"]:
                if row["flag"] == "UNMATCHED_PLACEHOLDER":
                    n_skipped += 1
                continue
    ...
    print(f"Assembled {n_done} images -> {args.out_docx} ({n_skipped} placeholders left unmatched)")
